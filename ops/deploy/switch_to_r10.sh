#!/bin/bash
# switch_to_r10.sh — 部署 r10 到生产 venv（r9b + 上游 7a369c9d 11 提交）+ 暖机兜底
# 回滚：bash scripts/rollback_r10.sh（恢复 ftllm.backup-20260921-pre-r10）
set -u
trap "" HUP
W=/home/ai-agent/builds/upgrade-test
V=/home/ai-agent/builds/fastllm-video-venv
PKG=$V/lib/python3.13/site-packages/ftllm
OVL=$W/overlay-r10/ftllm
BK=$V/lib/python3.13/site-packages/ftllm.backup-20260921-pre-r10
VPY=$V/bin/python
PLOG=/home/ai-agent/fastllm-video-repro/results/server-prod.service.log
SO_EXPECT=17586d35f49dc99fc622592d3bc0878a
PARSER_EXPECT=b253557592c542888568f44374a258df
exec > >(tee -a $W/switch-r10.log) 2>&1
echo "===== SWITCH-R10 START $(date '+%F %T') ====="

echo "--- PHASE0 preflight ---"
[ -f $OVL/libfastllm_tools.so ] || { echo NO_OVERLAY_SO; exit 2; }
M=$(md5sum $OVL/libfastllm_tools.so | cut -d' ' -f1); echo "overlay-r10 so md5=$M"
...
echo "--- PHASE2 sync overlay-r10 -> venv ---"
...
if [ "$RP" != "1" ]; then echo "PROD_NOT_READY — 回滚：bash $W/scripts/rollback_r10.sh"; exit 5; fi
...
$VPY $W/trial_probe2.py http://127.0.0.1:8080 $W/switch-r10-probe.json || echo PROBE_INCOMPLETE
echo "--- PHASE7 复跑 run2 + 错误扫描 ---"
$VPY $W/trial_probe2.py http://127.0.0.1:8080 $W/switch-r10-probe2.json || echo PROBE2_INCOMPLETE
...
echo "SWITCH_R10_DONE ready=$RP so=$M" > $W/switch-r10.status
cat $W/switch-r10.status
echo "===== SWITCH-R10 END $(date '+%F %T') ====="
