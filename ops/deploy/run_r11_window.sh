#!/bin/bash
# run_r11_window.sh — r11 冒烟窗口（2026-09-21）。生产暂让，脚本自动恢复。
# 探针: r11-force（dflash argv） / r11-ns（无投机 argv） / r11-ssd（现产同配+SSD env）
# md5 对照 r8 基线: f5de00c5 / 30f8a5c9ee88 / 2adaf2269e77
set -u
W=/home/ai-agent/builds/upgrade-test
V=/home/ai-agent/builds/fastllm-video-venv
VPY=$V/bin/python
LOG=$W/r11-window.log
STATUS=$W/r11-window.status
exec > >(tee -a "$LOG") 2>&1
echo "========= R11 WINDOW START $(date "+%F %T") ========="
echo "testing so=$(md5sum $W/overlay-r11/ftllm/libfastllm_tools.so | cut -d' ' -f1)"
PROD_OK=0
mkdir -p /home/ai-agent/prefix_ssd_r11test
...
stop_prod
probe $W/fastllm_test_launch_r11.py     r11-force argv-swift-trial.json
probe $W/fastllm_test_launch_r11_ns.py  r11-ns    argv-swift-trial-ns.json
probe $W/fastllm_test_launch_r11_ssd.py r11-ssd   argv-swift-trial.json
start_prod
echo "R11_WINDOW_DONE $(date "+%F %T") prod=$PROD_OK" > $STATUS
cat $STATUS
echo "========= R11 WINDOW END $(date "+%F %T") ========="
