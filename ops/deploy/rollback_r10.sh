#!/bin/bash
# rollback_r10.sh — 从 r10 回滚到 pre-r10（r9b）venv + 暖机
set -u
...
rm -rf $PKG
cp -a $BK $PKG
M=$(md5sum $PKG/libfastllm_tools.so | cut -d' ' -f1); echo "restored so md5=$M (expect 66f68772abe34a596bbe187addb0451a)"
...
echo "ROLLBACK_R10_DONE ready=$RP so=$M"
