#!/bin/bash
# switch_to_r9b.sh — 部署 r9b 到生产 venv（r9 上游全量合并 + PR#747）
# 回滚：bash scripts/rollback_r9b.sh（恢复 ftllm.backup-20260921-pre-r9b）
set -u
trap "" HUP
W=/home/ai-agent/builds/upgrade-test
V=/home/ai-agent/builds/fastllm-video-venv
PKG=$V/lib/python3.13/site-packages/ftllm
OVL=$W/overlay-r9b/ftllm
...
SO_EXPECT=66f68772abe34a596bbe187addb0451a
PARSER_EXPECT=b253557592c542888568f44374a258df
exec > >(tee -a $W/switch-r9b.log) 2>&1
echo "===== SWITCH-R9B START $(date '+%F %T') ====="

echo "--- PHASE0 preflight ---"
[ -f $OVL/libfastllm_tools.so ] || { echo NO_OVERLAY_SO; exit 2; }
M=$(md5sum $OVL/libfastllm_tools.so | cut -d' ' -f1); echo "overlay-r9b so md5=$M"
...
echo "--- PHASE2 sync overlay-r9b -> venv ---"
...
if [ "$RP" != "1" ]; then echo "PROD_NOT_READY — 回滚：bash $W/scripts/rollback_r9b.sh"; exit 5; fi
...
echo "SWITCH_R9B_DONE ready=$RP so=$M" > $W/switch-r9b.status
echo "===== SWITCH-R9B END $(date '+%F %T') ====="
