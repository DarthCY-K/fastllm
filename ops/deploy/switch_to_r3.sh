#!/bin/bash
# switch_to_r3.sh — 部署 r3（fork sm75-2080Ti @3f19b510 = 上游 74d3638 合并构建）到生产 venv。
# 回滚：bash scripts/rollback_r3.sh（恢复 ftllm.backup-20260915-pre-r3）
set -u
W=/home/ai-agent/builds/upgrade-test
V=/home/ai-agent/builds/fastllm-video-venv
PKG=$V/lib/python3.13/site-packages/ftllm
OVL=$W/overlay-r3/ftllm
...
SO_EXPECT=50bac23857803f748ac926f8bdba4874
PARSER_EXPECT=b253557592c542888568f44374a258df
exec > >(tee -a $W/switch-r3.log) 2>&1
echo "===== SWITCH-R3 START $(date '+%F %T') ====="
...
for s in ALLOW_YARN_WITH_DFLASH proposal_q selector_q FASTLLM_QWEN35_FINAL_CHUNK_DECODE_MAX FASTLLM_PREFIX_CACHE_SNAPSHOT_INTERVAL_PAGES; do
...
$VPY $W/post_switch_probe.py http://127.0.0.1:8080 $W/switch-r3-probe.json POST_R3 || echo PROBE_INCOMPLETE
echo "--- PHASE7 尾块≤64 回归（收敛式探针）---"
$VPY $W/r3_tail_probe2.py http://127.0.0.1:8080 $W/switch-r3-tail.json POST_R3_TAIL || echo TAIL_INCOMPLETE
...
echo "SWITCH_DONE ready=$RP pkg=r3 so=$M" > $W/switch-r3.status
echo "===== SWITCH-R3 END $(date '+%F %T') ====="
