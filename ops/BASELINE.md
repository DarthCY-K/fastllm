...
- **key 轮换 + 明文根因修复**：泄漏点 = `ftllm/openai_server/server.py` 的 `logging.info(args)`（每次重启把 64-hex key 写进日志；全机扫描 **70 个文件**含该 key）。修复 `ops/deploy/apply_nslog_redact.py`（幂等，只改打印对象、不动真实 args）；轮换 `ops/deploy/rotate_prod_key.sh` → `ok=1 new=200 old=401 noauth=401`、`new_key_leaks=0`。env 备份 `qwen38-0.2x.env.bak-pre-rotate-20260916`。
