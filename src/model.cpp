...
// printf("eos_token = %s\n", eos_token.c_str());
        ...
        if (model->YarnConfig() && (!dsparkPath.empty() || !dflashPath.empty())) {
            if (std::getenv("FASTLLM_ALLOW_YARN_WITH_DFLASH") == nullptr) {
            ...
            printf("[fastllm-experimental] Yarn + external DFlash/DSpark draft explicitly allowed by FASTLLM_ALLOW_YARN_WITH_DFLASH\n");
        }
